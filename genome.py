...
class Genome(object):
    ...
    def build_keys_db(self, stock):
        if stock.this == None:
            try: stock.parse()
            except Exception as e:
                print e
                return
        conn = stock.db.connect()
        print str(select([stock.this.c.key]))
        tmp = conn.execute(select([stock.this.c.key])).fetchall() #list, I hope?
        self.build_keys(tmp)
