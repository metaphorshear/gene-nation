...
class Pool(object):
    ...
    'Kate Beckinsale', 'Lara Croft', 'Bruce Campbell', 'Master Blaster', 'Romper Stomper', 'Flonominal Smith',
    'Sun Wukong', 'Liu Bei', 'Zhang Fei', 'Catelyn Tully', 'Sledge Hammer', 'Foxy Cleopatra']
    ...
    def new_pool(self):
        ...
        try:
            for stock in self.stocks:
                print stock.symbol, stock.parsed
    ...
    def parse_stocks(self):
        ...
        for s in self.stocks:
            if len(s.csv) > 0 and len(s.price_data) >0:
                assert(len(s.csv) != 0)
                s.parse()
                s.update(self.start_date)
            else:
                self.stocks.remove(s)
                assert(s not in self.stocks)
                self.pick_symbols()
        return self.stocks
    ...
    def run_pool(self, iterations=1, samespan=True, newspan=None, incest=True, mutation=False, mercy=None, crotchety=False):
        """
